      ...
      <section className="main-grid">
        <div className="moments-zone">
          ...
          <div className="moment-grid">
            {moments.map((moment: Moment, index: number) => <MomentCard moment={moment} index={index} me={home.me.displayName} key={moment._id} />)}
            ...
            {Array.from({ length: Math.max(0, 3 - moments.length - (moments.some((m: Moment) => m.creatorName === home.me.displayName) ? 0 : 1)) }).map((_, index: number) => <div className="empty-card" key={index}><span>✦</span> waiting for a<br />little thing</div>)}
          </div>
        </div>
...
function Archive({ songs, onBack }: { songs: Array<{ _id: string; title: string; day: string; momentIds: string[]; strudelCode?: string | null; moments: Array<{ role: Role; soundUrl?: string | null; strudelCode?: string | null }> }>; onBack: () => void }) {
  return <section className="archive"><button className="back-link" onClick={onBack}>← back to today</button><p className="kicker">A RECORD OF THE ORDINARY STUFF</p><h1>small <em>archive.</em></h1><p className="archive-copy">Songs from the days you would otherwise<br />forget were this good.</p><div className="archive-list">{songs.length ? songs.map((song, index: number) => <article key={song._id}><span>0{index + 1}</span><div className={`archive-art art-${index % 4}`}>♫</div><div><small>{new Date(`${song.day}T12:00:00`).toLocaleDateString("en", { month: "short", day: "numeric" })} · {song.momentIds.length} moments</small><h2>{song.title}</h2></div><BopPlayButton moments={song.moments} code={song.strudelCode} className="archive-play" /></article>) : <div className="archive-empty">Your first bop will live here. It&apos;ll be nice to meet it again.</div>}</div></section>;
}
